...
| 50 | Bulk sed fails silently on heterogeneous patterns | Route middleware chains vary (`requireAdmin`, `csrfProtection`, direct `async`). Generic sed patterns miss routes silently. After bulk sed, always verify with counter grep to catch unwired targets. Never assume uniform signatures | SPT |
| 51 | Version bump without frontend rebuild | APP_VERSION is baked into Vite/webpack build artifacts. Changing source without rebuilding leaves UI showing old version. Every version bump must rebuild ALL frontend containers | SPT |
| 52 | Cross-component deps break naive extraction | Pages that import other pages or share utility constants (Icons, formatCurrency) cannot be extracted by simple line-range scripts. Need full dependency graph (JSX refs + constant refs + function refs) before automated extraction. If pages cross-reference, refactor routing first | SPT |
